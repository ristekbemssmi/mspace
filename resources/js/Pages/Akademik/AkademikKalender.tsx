        ...
        <div className="flex flex-col items-center bg-[#E2E2E2] rounded-xl overflow-hidden shadow-2xl border border-gray-300">
            ...
            <div className="w-full bg-linear-to-b from-white to-[#F4E06D] border-b border-gray-300 p-3 flex flex-wrap items-center justify-between gap-4 sticky top-0 z-10">
                ...
                <div className="flex items-center gap-1 bg-gray-100 p-1 rounded-lg">
                    <button type="button" aria-label="Perkecil zoom" onClick={() => setScale(s => Math.max(s - 0.2, 0.5))} className="p-1.5 hover:bg-white rounded-md shadow-sm transition-all"><ZoomOut size={18} /></button>
                    <span className="text-xs font-bold w-12 text-center">{Math.round(scale * 100)}%</span>
                    <button type="button" aria-label="Perbesar zoom" onClick={() => setScale(s => Math.min(s + 0.2, 2.0))} className="p-1.5 hover:bg-white rounded-md shadow-sm transition-all"><ZoomIn size={18} /></button>
                </div>

                <div className="flex items-center gap-2">
                    <button type="button" onClick={handleDownload} className="p-2 hover:bg-blue-50 text-blue-600 rounded-lg transition-colors" title="Download">
                        <Download size={20} />
                    </button>
                    <button type="button" onClick={() => window.print()} className="p-2 hover:bg-gray-100 rounded-lg transition-colors" title="Print">
                        <Printer size={20} />
                    </button>
                </div>
            </div>
