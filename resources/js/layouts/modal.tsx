...
const Modal = ({ isOpen, onClose, title, children }: ModalProps) => {
    ...
    useEffect(() => {
        if (isOpen) {
            setIsRendered(true);
            // Slight delay to allow DOM to render before adding visible classes to trigger animation
            const timer = setTimeout(() => setIsVisible(true), 10);
            return () => clearTimeout(timer);
        } else {
            setIsVisible(false);
            // Wait for transition to finish before unmounting (300ms matches Tailwind duration)
    ...
    return (
        // Backdrop overlay: covers entire screen and centers content
        <div
        ...
        >
            {/* Modal Container: prevents event bubbling so clicking inside doesn't close it */}
            <div
