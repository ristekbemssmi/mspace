...
export default function BeasiswaSection({ data = [] }: { data: BeasiswaItem[] }) {
        ...
        return (
            ...
            >
                ...
                <div className="w-full md:w-1/2 h-48 md:h-full relative overflow-hidden shrink-0">
                    <img
                        src={item.beasiswa?.link_poster || '/img/beasiswa.webp'}
                        alt={item.judul}
                        width="600"
                        height="400"
                        loading="lazy"
                ...
                <div className={`flex-1 p-8 md:p-12 text-left flex flex-col justify-center gap-4 transition-opacity duration-500 ${isActive ? 'opacity-100' : 'opacity-0'}`}>
                    ...
                    <div className="flex flex-wrap gap-3 mt-4">
                        <button type="button" className="flex items-center gap-2 bg-white/10 hover:bg-white/20 text-white px-5 py-2.5 rounded-xl border border-white/10 transition-all font-semibold text-sm">
